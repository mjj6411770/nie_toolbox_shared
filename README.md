...
## Filter Scaling Improvement

- Changed the scaling factor calculation in the signal filtering process to use RMS (Root Mean Square) values instead of simple mean/median ratios
- This improves amplitude information preservation in the filtered signal
- The scale factor is now calculated as: `scale_factor = rms(original_signal) / rms(filtered_signal)`
- This change helps maintain better signal amplitude accuracy after filtering
- Implementation can be found in [`SignalAnalysis.m`](test_simple/SignalAnalysis.m) lines 248-265
...
When you run the code, it will show two plots which are explained in "Final spike feature extraction" section below. The first plot is the original signal with the marked spikes, and the second plot is the statistic of the spikes. The statistic includes the number of spikes, the average height, the average area, and the average prominence of the spikes.
